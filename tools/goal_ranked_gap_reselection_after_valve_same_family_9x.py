...
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_valve_same_family_9x"
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.17 Ranked Gap Reselection After Valve Same-family Audit",
        "",
        "Dev-only reselection from wrong-rank buckets after removing exhausted, blocked, and label-heavy valve directions. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.17 ranked gap reselection after valve same-family audit")
    ...
    blocked_group_ids = fire_damper_group_ids | valve_duct_group_ids | valve_same_group_ids
    ...
    for row in all_wrong_rank:
        if _clean(row.get("group_id")):
            all_group_ids.add(_clean(row.get("group_id")))
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.17 ranked gap reselection after valve same-family audit",
        "read_only": True,
        "eval_only": True,
        ...
        "decision": (
            ...
            "duct fire-damper review rows, single-source valve/duct size-tier rows, and the label-heavy valve same-family bucket. "
        ...
        "anti_drift_conclusion": "Stage 9.17 only reselects the next dev audit bucket. It subtracts the blocked valve same-family rows and does not use heldout, train, tune, write rules, or change GoalSearcher.",
        "next_stage": {
            "stage": f"9.18 {selected['query_family']} {selected['reason']} audit",
