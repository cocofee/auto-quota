...
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_lamp_near_miss_9x"
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.21 Ranked Gap Reselection After Lamp Near-miss Audit",
        "",
        "Dev-only reselection from wrong-rank buckets after removing exhausted, blocked, label-heavy, and single-source-heavy lamp directions. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.21 ranked gap reselection after lamp near-miss audit")
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.21 ranked gap reselection after lamp near-miss audit",
        "read_only": True,
        "eval_only": True,
        ...
        "decision": (
            ...
            "duct fire-damper rows, blocked valve directions, lamp same-family rows, and the single-source-heavy lamp near-miss bucket. "
        ...
        "anti_drift_conclusion": "Stage 9.21 only reselects the next dev audit bucket. It subtracts the blocked lamp near-miss rows and does not use heldout, train, tune, write rules, or change GoalSearcher.",
        "next_stage": {
            "stage": f"9.22 {selected['query_family']} {selected['reason']} audit",
