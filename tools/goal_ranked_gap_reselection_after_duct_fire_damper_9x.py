...
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_duct_fire_damper_9x"
...
def _load_exhausted_group_ids(path: Path) -> set[str]:
    if not path.exists():
        return set()
    return {_clean(row.get("group_id")) for row in _read_csv(path) if _clean(row.get("group_id"))}
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.12 Ranked Gap Reselection After Duct Fire-damper",
        "",
        "Dev-only reselection from wrong-rank buckets after removing exhausted duct fire-damper rows. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.12 ranked gap reselection after duct fire-damper")
    ...
    exhausted_group_ids = _load_exhausted_group_ids(Path(args.fire_damper_rows))
    all_wrong_rank = [
        row
    ...
    remaining = [row for row in all_wrong_rank if _clean(row.get("group_id")) not in exhausted_group_ids]
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.12 ranked gap reselection after duct fire-damper",
        "read_only": True,
        "eval_only": True,
        ...
        "exhausted_artifact": str(Path(args.fire_damper_rows)),
        "metrics": {
            "dev_wrong_rank_rows": len(all_wrong_rank),
            "excluded_fire_damper_rows": len(all_wrong_rank) - len(remaining),
        ...
        "decision": "Select valve + near_miss_rank_2_5 after removing exhausted duct fire-damper rows and excluding already-exhausted pipe and electrical_box near-miss directions. It has support=24, spans 5 provinces and 4 sources, and all selected rows are rank_2_5, so the next step should be a read-only valve near-miss audit rather than a rule change.",
        "artifacts": artifacts,
        "elapsed_sec": round(time.perf_counter() - started, 3),
        "anti_drift_conclusion": "Stage 9.12 only reselects the next dev audit bucket. It subtracts the exhausted duct fire-damper rows and does not use heldout, train, tune, or implement valve rules. Valve must be audited in 9.13 before any what-if or feature design.",
        "next_stage": {
            "stage": "9.13 valve near-miss audit",
            "goal": "audit the 24 dev valve near_miss_rank_2_5 rows and split them into soft-joint/compensator, fire-damper/control-valve leakage, DN/size tier evidence, valve subtype, book/chapter bias, and label-insufficient buckets",
            "prohibited": [
                "training",
                "tuning",
                "rule patches",
