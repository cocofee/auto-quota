...
DEFAULT_HELDOUT = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_validation_heldout_summary.json"
DEFAULT_HARD = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_validation_hard_summary.json"
DEFAULT_OUTPUT = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_validation_package_review_summary.json"
...
def _decision(split_reports: list[dict[str, Any]]) -> tuple[str, str]:
    ...
    if any(status.startswith("fail") for status in statuses):
        return "validation_failed_do_not_release", "At least one validation split failed rank1, Hit5, or Top1 safety."
...
def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    for split_report in report["split_reports"]:
        ...
        rows.append([m["split"], f"{m['hit1_gain']}/{m['hit1_loss']}/{m['hit1_net']}", m["rank1_loss_count"], m["hit5_net"], m["applied_groups"], _split_status(m)])
    lines = [
        "# 14.14 R14V2_E Heldout/Hard Validation Package Review",
        "",
...
def _update_status(path: Path, report: dict[str, Any]) -> None:
    ...
    text = "\n".join(
        [
            ...
            "## Where We Are",
            "",
            "Current stage: **14.14 heldout/hard A/B validation completed**.",
            "",
            f"Decision: **{report['decision']}**.",
            "",
            ...
            *rows,
            ...
            "## Current Boundary",
            "",
            "Default: `do_not_release` unless a separate release/integration gate is explicitly authorized.",
            "",
            ...
            "- changing candidate or gates using heldout/hard feedback",
            "",
            "Next recommended stage: **14.15 post-validation release/closure gate**.",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _update_dashboard(path: Path, report: dict[str, Any]) -> None:
    ...
    prompt = (
        "按 Goal Roadmap 看板执行。\n"
        "当前阶段：14.14 heldout/hard A/B validation for frozen R14V2_E 已完成。\n"
        ...
        "下一步建议：14.15 post-validation release/closure gate。默认不 release、不接 GoalSearcher、不用 heldout/hard 反调候选。\n"
    ...
    if "14.14 R14V2_E heldout/hard validation package review" not in text:
        row = f"""          <tr>
            <td>14.14 R14V2_E heldout/hard validation package review</td>
            <td>Fixed R14V2_E validation package across heldout and hard; no tuning, no release, no GoalSearcher change.</td>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="14.14 R14V2_E heldout/hard validation package review")
    ...
    report = {
        "stage": "14.14 heldout/hard A/B validation package review for frozen R14V2_E",
        ...
        "anti_drift_conclusion": "14.14 reviewed validation outputs only. It did not tune thresholds, switch candidates, expand gates, release code, edit GoalSearcher, or use heldout/hard for model selection.",
        "next_stage": {"recommended": "14.15 post-validation release/closure gate", "default": "do_not_release"},
    }
