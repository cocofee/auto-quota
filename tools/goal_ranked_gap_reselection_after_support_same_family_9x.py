...
DEFAULT_LAMP_NEAR_ROWS = PROJECT_ROOT / "reports" / "agent_state" / "goal_lamp_near_miss_9x_audit_rows.csv"
DEFAULT_SUPPORT_SAME_ROWS = PROJECT_ROOT / "reports" / "agent_state" / "goal_support_same_family_unknown_9x_audit_rows.csv"
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_support_same_family_9x"
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.23 Ranked Gap Reselection After Support Same-family Audit",
        "",
        "Dev-only reselection from wrong-rank buckets after removing exhausted, blocked, label-heavy, single-source-heavy, and mixed support directions. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
            [
                ...
                ["excluded_lamp_near_miss_rows", report["metrics"]["excluded_lamp_near_miss_rows"]],
                ["excluded_support_same_family_rows", report["metrics"]["excluded_support_same_family_rows"]],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.23 ranked gap reselection after support same-family audit")
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.23 ranked gap reselection after support same-family audit",
        "read_only": True,
        "eval_only": True,
        ...
        "blocked_lamp_near_artifact": str(Path(args.lamp_near_rows)),
        "blocked_support_same_artifact": str(Path(args.support_same_rows)),
        "metrics": {
        ...
        "decision": (
            (
                ...
                "duct fire-damper rows, blocked valve directions, lamp same-family and near-miss rows, and the mixed support same-family bucket. "
            ...
            if eligible
                ...
                "Stop reselection: after excluding exhausted pipe/electrical_box directions, duct fire-damper rows, blocked valve directions, "
                "lamp same-family and near-miss rows, and the mixed support same-family bucket, no remaining dev wrong-rank bucket passes the "
                "support/province/source selection gate. The broad <empty> family bucket remains blocked and must not be selected as an audit bucket."
            )
        ),
        "artifacts": artifacts,
        "elapsed_sec": round(time.perf_counter() - started, 3),
        "anti_drift_conclusion": "Stage 9.23 only reselects the next dev audit bucket. It subtracts the blocked support same-family rows and does not use heldout, train, tune, write rules, or change GoalSearcher.",
        "next_stage": {
            "stage": (
                f"9.24 {selected['query_family']} {selected['reason']} audit"
                if eligible
                else "9.24 no eligible dev wrong-rank bucket gate review"
            ),
            "goal": (
                f"audit the {selected['count']} dev rows in {selected['query_family']} + {selected['reason']} and split them into transferable evidence, label issues, same-source artifacts, and ranking/parameter buckets"
                if eligible
                else "review the no-eligible-candidate state and decide whether to decompose the blocked query_family_empty bucket or explicitly adjust the selection gate"
            ),
            "prohibited": [
                "training",
                "tuning",
