...
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_valve_duct_block_9x"
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.15 Ranked Gap Reselection After Valve/Duct Block",
        "",
        "Dev-only reselection from wrong-rank buckets after removing exhausted and blocked directions. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.15 ranked gap reselection after valve/duct block")
    ...
    blocked_group_ids = fire_damper_group_ids | valve_duct_group_ids
    all_wrong_rank = [
        row
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.15 ranked gap reselection after valve/duct block",
        "read_only": True,
        "eval_only": True,
        ...
        "metrics": {
            ...
            "excluded_fire_damper_rows": len(fire_damper_group_ids & {_clean(row.get("group_id")) for row in all_wrong_rank}),
            "excluded_valve_duct_block_rows": len(valve_duct_group_ids & {_clean(row.get("group_id")) for row in all_wrong_rank}),
            "excluded_group_ids_total": len(blocked_group_ids & {_clean(row.get("group_id")) for row in all_wrong_rank}),
        ...
        "decision": (
            ...
            "duct fire-damper review rows, and the single-source valve/duct size-tier rows. "
        ...
        "anti_drift_conclusion": "Stage 9.15 only reselects the next dev audit bucket. It subtracts blocked single-source valve/duct rows and does not use heldout, train, tune, write rules, or change GoalSearcher.",
        "next_stage": {
            "stage": f"9.16 {selected['query_family']} {selected['reason']} audit",
