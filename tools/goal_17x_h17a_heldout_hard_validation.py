...
DEFAULT_FREEZE = AGENT_STATE / "goal_17x_h17a_freeze_gate_validation_boundary_summary.json"
DEFAULT_AUTH = AGENT_STATE / "goal_17x_h17a_validation_go_no_go_summary.json"
DEFAULT_PREFIX = AGENT_STATE / "goal_17x_h17a_heldout_hard_validation"
...
def _stop_conditions(all_head: dict[str, Any], scorecard: list[dict[str, Any]], resolved_provinces: int, db_dir: Path) -> list[dict[str, str]]:
    ...
    support = next((row for row in scorecard if row.get("slice") == "family:support"), {})
    pipe_support_generated = _safe_int(pipe.get("prior_generated_candidates")) + _safe_int(support.get("prior_generated_candidates"))
    return [
        {"check": "validation_substrate", "status": "pass", "evidence": f"resolved_provinces={resolved_provinces}, db_dir={db_dir}"},
        {"check": "frozen_candidate_contract", "status": "pass", "evidence": "candidate=H17_A; core=concrete,pump,rebar; pipe/support vetoed"},
        {"check": "pipe_support_veto", "status": "pass" if pipe_support_generated == 0 else "fail", "evidence": f"pipe_support_prior_generated_candidates={pipe_support_generated}"},
...
def _decision(stop_conditions: list[dict[str, str]]) -> str:
    ...
    if failed:
        return "validation_stop_do_not_release_h17a"
...
def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# 17.17 H17_A Heldout/Hard Validation",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="17.17 heldout/hard validation for frozen H17_A only")
    ...
    if freeze.get("decision") != "freeze_h17a_for_validation_request_boundary":
        raise ValueError(f"H17_A is not frozen for validation: {freeze.get('decision')}")
    auth = json.loads(args.authorization_summary.read_text(encoding="utf-8"))
    if auth.get("decision") not in {"validation_authorized_for_next_execution_stage", "do_not_validate_yet"}:
        raise ValueError(f"unexpected H17_A authorization state: {auth.get('decision')}")
    ...
    if decision.startswith("validation_pass"):
        interpretation = (
            "Frozen H17_A passed the heldout/hard validation stop conditions. It is still not default-enabled; next step is a post-validation integration/release gate."
        )
    else:
        interpretation = (
            "Frozen H17_A did not pass heldout/hard validation. Keep it default-off and return to broader recall/index strategy rather than tuning on heldout/hard."
        )
    ...
    report = {
        "stage": "17.17 frozen H17_A heldout/hard validation",
        "decision": decision,
        ...
        "next_stage": "17.18 post-validation integration/release gate" if decision.startswith("validation_pass") else "17.18 validation-failed closure / strategy return",
        "artifacts": {
        ...
        "anti_drift_conclusion": (
            "17.17 ran only the explicitly authorized heldout/hard A/B validation for frozen H17_A. "
            "It did not train, tune from validation, change thresholds, reintroduce pipe/support, default-enable OSS recall, integrate online behavior, or change GoalSearcher defaults."
        ),
