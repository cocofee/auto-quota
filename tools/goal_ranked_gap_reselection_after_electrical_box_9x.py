...
DEFAULT_OUTPUT_PREFIX = PROJECT_ROOT / "reports" / "agent_state" / "goal_ranked_gap_reselection_after_electrical_box_9x"
...
def _write_markdown(path: Path, report: dict[str, Any], candidates: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Stage 9.9 Ranked Gap Reselection After Electrical Box",
        "",
        "Dev-only reselection from wrong-rank buckets after exhausting the electrical_box near-miss direction. No training, tuning, rule patch, ranking change, heldout selection, or GoalSearcher change.",
        "",
        "## Summary",
        "",
        _md_table(
            [
                ...
                ["excluded_candidates", report["metrics"]["excluded_candidates"]],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 9.9 ranked gap reselection after electrical_box")
    ...
    report = {
        "stage": "Goal LTR v1 / stage 9.9 ranked gap reselection after electrical_box",
        "read_only": True,
        "eval_only": True,
        ...
        "decision": "Select duct + near_miss_rank_2_5 for the next audit after excluding exhausted pipe and electrical_box near-miss directions. It has support=20, spans 8 provinces and 4 sources, and all selected rows are rank_2_5, so the next step should be a read-only near-miss audit rather than a rule change.",
        "artifacts": artifacts,
        "elapsed_sec": round(time.perf_counter() - started, 3),
        "anti_drift_conclusion": "Stage 9.9 only reselects the next dev audit bucket. It excludes already-exhausted directions and does not use heldout, train, tune, or implement duct rules. Duct must be audited in 9.10 before any what-if or feature design.",
        "next_stage": {
            "stage": "9.10 duct near-miss audit",
            "goal": "audit the 20 dev duct near_miss_rank_2_5 rows and split them into size/diameter evidence, rectangular/round subtype, fabrication/install wording, book/chapter bias, and label-insufficient buckets",
            "prohibited": [
                "training",
                "tuning",
                "rule patches",
