...
DEFAULT_FOLD_MANIFEST = AGENT_STATE / "goal_13x_expanded_matrix_guarded_reranker_dev_oof_fold_manifest.csv"
...
def _num_boost_round(fold_manifest_path: Path, candidate_id: str, default: int = 120) -> int:
    if not fold_manifest_path.exists():
        return default
    rows = _read_csv(fold_manifest_path)
    values = [_int(row.get("num_boost_round")) for row in rows if row.get("candidate_id") == candidate_id and _int(row.get("num_boost_round")) > 0]
...
def _train_fixed_candidate(
    *,
    ...
    train_weights = _group_weights(candidate["objective_variant"], labels, groups)
    ...
    booster = lgb.train(
        ...
        valid_names=["dev_train_fixed_candidate"],
        callbacks=[],
    )
...
def _build_validation_matrix(rows: list[dict[str, Any]], split: str, top_k: int) -> tuple[pd.DataFrame, np.ndarray, list[int], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    return pd.DataFrame(feature_rows), np.array(labels, dtype=np.int8), groups, meta, details, recall_missing_rows
...
def _score_split(
    *,
    ...
    for group_idx, size in enumerate(groups):
        ...
        gain_flag = bool((not base_h1) and cand_h1)
        loss_flag = bool(base_h1 and not cand_h1)
        ...
        top_feature = feature_rows[global_top_idx]
        group_meta = meta[group_idx]
        candidate_top_family = clean_text(top_feature.get("candidate_family")) or "<empty>"
        candidate_top_book = clean_text(top_feature.get("quota_book")) or "<empty>"
        dims = {
            "province": clean_text(group_meta.get("province")) or "<empty>",
            "source_file": clean_text(group_meta.get("source_file")) or "<empty>",
            "bucket": clean_text(group_meta.get("bucket")) or "<empty>",
            "query_family": clean_text(group_meta.get("query_family")) or "<empty>",
        ...
        detail_rows.append(
            {
                ...
                "baseline_hit5": bool(base_h5),
                "candidate_hit5": bool(cand_h5),
                "flip_type": "gain" if gain_flag else "loss" if loss_flag else "neutral",
                "candidate_top_quota_id": clean_text(top_feature.get("quota_id")),
    ...
    loss_rows = [row for row in detail_rows if row["flip_type"] == "loss"]
    gain_rows = [row for row in detail_rows if row["flip_type"] == "gain"]
    source_gain = Counter(row["province"] for row in gain_rows)
    max_province_gain = max(source_gain.values(), default=0)
    metrics = {
        "split": split,
        "candidate_id": candidate_id,
        "rows": total_rows,
        ...
        "new_loss_count": len(loss_rows),
        "max_province_gain_share": round(max_province_gain / len(gain_rows), 6) if gain_rows else 0.0,
    ...
    source_rows = [
        {"candidate_id": candidate_id, "split": split, "slice_dimension": dimension, "slice_key": key, **values}
        for (dimension, key), values in source_stats.items()
    ]
    source_rows.sort(key=lambda row: (row["net"], row["gain"], row["groups"]), reverse=True)
    slice_rows.sort(key=lambda row: (row["loss"], row["groups"]), reverse=True)
    return metrics, detail_rows, slice_rows, source_rows


def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    m = report["metrics"]
    lines = [
        f"# 13.14 {m['split']} A/B Validation",
        "",
        "Heldout/hard validation for the frozen 13.10 expanded reranker candidate. The candidate is fixed; this run does not use validation splits for selection, training choices, threshold tuning, or online integration.",
        "",
        "## Summary",
        "",
        _md_table(
            [
                ...
                ["baseline_hit1_rate", m["baseline_hit1_rate"]],
                ["candidate_hit1_rate", m["candidate_hit1_rate"]],
                ["hit5 gain/loss/net", f"{m['hit5_gain']}/{m['hit5_loss']}/{m['hit5_net']}"],
                ["max_province_gain_share", m["max_province_gain_share"]],
            ]
        ),
        "",
        "## Anti-Drift",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="13.14 heldout/hard A/B validation for frozen expanded reranker candidate")
    ...
    num_boost_round = _num_boost_round(args.fold_manifest, args.candidate_id)
    ...
    df, labels, groups, meta, _details, recall_missing_rows = _build_validation_matrix(rows, args.split, args.top_k)
    feature_rows = df.to_dict("records")
    elapsed = time.perf_counter() - started
    metrics, detail_rows, loss_slice_rows, source_slice_rows = _score_split(
        split=args.split,
        candidate_id=args.candidate_id,
        booster=booster,
        features=features,
    ...
    report = {
        "stage": f"13.14 {args.split} heldout/hard A/B validation",
        ...
        "metrics": metrics,
        "artifacts": artifacts,
        "anti_drift_conclusion": "Validation execution only: fixed frozen candidate, no candidate reselection, no hyperparameter tuning, no matrix expansion, no online integration, no threshold change, and no GoalSearcher edit.",
    }
    _write_json(Path(artifacts["summary_json"]), report)
    _write_markdown(Path(artifacts["summary_md"]), report)
    _write_jsonl(Path(artifacts["details_jsonl"]), detail_rows)
    _write_jsonl(Path(artifacts["recall_missing_jsonl"]), recall_missing_rows)
    _write_csv(Path(artifacts["loss_slices_csv"]), loss_slice_rows, ["candidate_id", "split", "slice_dimension", "slice_key", "groups", "baseline_hit1", "candidate_hit1", "gain", "loss", "net", "baseline_hit1_rate", "candidate_hit1_rate"])
    _write_csv(Path(artifacts["source_slices_csv"]), source_slice_rows, ["candidate_id", "split", "slice_dimension", "slice_key", "groups", "gain", "loss", "net"])
