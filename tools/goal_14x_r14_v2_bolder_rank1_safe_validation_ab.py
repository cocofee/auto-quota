...
from tools.goal_10x_offline_ranking_experiment_dev_oof_execute import _load_dev_matrix, _load_training_features  # noqa: E402
...
from tools.goal_14x_r14_v2_bolder_rank1_safe_dev_oof_execute import (  # noqa: E402
    _candidate_features,
    _gate_decision_v2,
    _group_weights,
    _objective_params,
...
DEFAULT_CANDIDATE_PLAN = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_redesign_plan_candidate_matrix.csv"
DEFAULT_FROZEN_MANIFEST = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_freeze_gate_review_frozen_candidate.json"
DEFAULT_DEV_OOF_SUMMARY = AGENT_STATE / "goal_14x_r14_v2_bolder_rank1_safe_dev_oof_execution_summary.json"
...
def _source_family(row: dict[str, Any]) -> str:
    ...
    return f"{_clean(row.get('province'))}|{_clean(row.get('source_file'))}"


def _load_candidate(candidate_plan: Path, candidate_id: str) -> dict[str, Any]:
    for row in _read_csv(candidate_plan):
        if row.get("candidate_id") == candidate_id:
            return dict(row)
    raise ValueError(f"candidate not found in plan: {candidate_id}")
...
def _train_fixed_candidate(
    *,
    ...
    booster = lgb.train(
        ...
        valid_names=["dev_train_fixed_r14v2"],
        callbacks=[],
    )
...
def _score_split(
    *,
    ...
    for group_idx, size in enumerate(groups):
        ...
        if candidate_id == "R14V2_E_rank1_shadow_no_demote" and baseline_rank == 1:
    ...
    loss_rows = [
        {"candidate_id": candidate_id, "split": split, "slice_dimension": dimension, "slice_key": key, **values, "baseline_hit1_rate": round(values["baseline_hit1"] / values["groups"], 6) if values["groups"] else 0.0, "candidate_hit1_rate": round(values["candidate_hit1"] / values["groups"], 6) if values["groups"] else 0.0}
    ...
    return metrics, detail_rows, recall_missing_rows, loss_rows, gate_rows, source_rows, contract_rows


def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    m = report["metrics"]
    lines = [
        f"# 14.14 {m['split']} R14V2_E A/B Validation",
        "",
        "Heldout/hard A/B validation for frozen R14V2_E. Candidate, objective, features, and thresholds are fixed from dev/OOF; validation is not used for selection or tuning.",
        "",
        "## Summary",
        "",
        _md_table(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="14.14 heldout/hard A/B validation for frozen R14V2_E")
    ...
    if args.candidate_id != "R14V2_E_rank1_shadow_no_demote":
        raise ValueError("14.14 is scoped only to frozen R14V2_E")
    if manifest.get("freeze_status") != "freeze_candidate_for_future_validation":
        raise ValueError(f"candidate is not frozen for validation: {manifest.get('freeze_status')}")

    candidate = _load_candidate(args.candidate_plan, args.candidate_id)
    thresholds = _load_fixed_thresholds(args.dev_oof_summary)
    _configure_db_dirs(args.db_dir)
    booster, features, train_metrics = _train_fixed_candidate(data_dir=args.data_dir, feature_whitelist=args.feature_whitelist, candidate=candidate, num_boost_round=args.num_boost_round, seed=args.seed)
    ...
    report = {
        "stage": f"14.14 {args.split} heldout/hard A/B validation for frozen R14V2_E",
        ...
        "metrics": metrics,
        "artifacts": artifacts,
        "anti_drift_conclusion": "14.14 validation only: fixed R14V2_E, fixed dev/OOF thresholds, no heldout/hard selection or tuning, no candidate switch, no release, no GoalSearcher edit, and no online threshold change.",
    }
