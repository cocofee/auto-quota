...
def save_results_to_db(session, task_id: uuid.UUID, results: list[dict]):
    ...
    for idx, result in enumerate(results):
        ...
        match_result = MatchResult(
            task_id=task_id,
            index=idx,
            bill_code=bill_item.get("code", "")[:50],
            bill_name=bill_item.get("name", "")[:500],
            bill_description=bill_item.get("spec", bill_item.get("description", "")),
            bill_unit=bill_item.get("unit", "")[:50],
            bill_quantity=bill_item.get("quantity"),
            specialty=bill_item.get("specialty", "")[:20],
            sheet_name=bill_item.get("sheet_name", "")[:100],
            section=bill_item.get("section", "")[:200],
