...
| B2 电气 | 283条 | 大题量，电气专业（配电箱/配管/电缆/灯具） |
| B3 配套楼混合 | 95条 | 多专业混在一起（给排水+电气+消防+通风） |
| B4 脏数据 | 19条 | 故意放的烂数据，测系统容错能力 |

### 历史成绩

> 旧版Excel试卷(B1-B4)的历史成绩，现已被11省2174条JSON试卷替代。
> 当前基线和历史趋势见 `_AI备忘录.md`。
...
| | 本地开发（.env） | Docker生产（docker-compose覆盖） |
|---|---|---|
| 匹配模型（AGENT_LLM） | Claude Opus 4.6 | 千问(qwen-plus) |
| 验证模型（VERIFY_LLM） | Claude（跟匹配走） | Kimi(k2.5) |

Docker容器的 `environment` 优先级高于 `env_file`（.env），所以本地和生产自动切换，不用手动改。

### 本地开发端口

| 服务 | 端口 | 说明 |
|------|------|------|
| 前端（Docker/Nginx） | 3210 | 3000被Hyper-V保留，改用3210 |
| 前端（Vite开发） | 5173 | 本地开发热重载 |
| 后端（FastAPI） | 8000 | API + Swagger文档 |
| PostgreSQL | 5432 | 用户名autoquota |
| Redis | 6379 | Celery任务队列（容器内部，未映射到宿主机） |
...
常用测试文件：
- 公厕给排水：`D:\广联达临时文件\2025\公厕-给排水.xlsx`
- 7#配套楼：`output\temp\7#配套楼.xlsx`

## Compact Instructions

上下文压缩时，按以下优先级保留信息（绝不能丢的排最前面）：

1. **架构决策**（绝不压缩）：两层知识体系、两层数据质量机制、匹配流程7步、经验库权威层/候选层区别
2. **关键设计决策**：不依赖编码、经验库只存人工验证、一省通全国、定额版本绑定
3. **当前改了哪些文件**：文件路径 + 改动要点
4. **质量门禁状态**：pytest/健康检查/benchmark 各自通过还是失败
5. **待办和风险**：未完成的TODO、已知的回滚要点、P0/P1问题
6. **工具调用结果**（可删除，只保留通过/失败结论）
