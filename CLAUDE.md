...
- 导入更多种子数据到经验库和通用知识库
- 土建/市政定额导入（等用户从广联达导出）
- **界面迁移：Streamlit → NiceGUI**（优先级：算法稳定后再做）
  - 原因：Streamlit 交互体验差（整页刷新、表格不能直接编辑、状态易丢失）
  - 方案：用 NiceGUI 替代，纯 Python 框架，支持实时交互、表格编辑、拖拽上传
  - 备选：Reflex（Python 写但编译成 React）、Flet（可打包成 exe 桌面应用）
  - 迁移范围：4个 Streamlit 页面（匹配定额、定额数据库、经验库、设置）
  - 注意：核心匹配引擎（src/）不需要改，只替换界面层
