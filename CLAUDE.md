...
全局规则已定义 src/data/output 结构，以下是本项目额外约定：
- 测试和评测工具放在 `tests/` 下
- 辅助导入工具放在 `tools/` 下
...
3. 运行验收命令并给出结果。
4. 输出以下内容：
   - 修改文件列表 + 每个文件关键改动（不超过3行）
   - 验收命令与结果
   - 风险点与回滚点
   - `git diff --stat`
   - **提醒用户运行代码审查**：`代码已改完，请双击"代码审查.bat"运行 Codex 5.3 审查。`
...
- 导入更多种子数据到经验库和通用知识库
- 土建/市政定额导入（等用户从广联达导出）
- 界面迁移：Streamlit → NiceGUI（算法稳定后再做）
