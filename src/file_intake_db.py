...
class FileIntakeDB:
    ...
    def update_classify(self, file_id: str, *, file_type: str, classify_result: dict) -> dict | None:
        ...
        try:
            conn.execute(
                ...
                WHERE file_id=?
                """,
                (file_type, _json_dump(classify_result), now, file_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def update_parse(self, file_id: str, *, status: str, parse_summary: dict) -> dict | None:
        ...
        try:
            conn.execute(
                ...
                WHERE file_id=?
                """,
                (status, _json_dump(parse_summary), now, file_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def update_route(self, file_id: str, *, route_result: dict) -> dict | None:
        ...
        try:
            conn.execute(
                ...
                WHERE file_id=?
                """,
                (_json_dump(route_result), now, file_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def update_failure(self, file_id: str, *, error_message: str) -> dict | None:
        now = time.time()
        conn = db_connect(self.db_path)
        try:
            conn.execute(
